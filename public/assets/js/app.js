...
const ResumeBuilder = {
    ...
    async init(id) {
        ...
        app.innerHTML = Layout.render(`
            ...
            <div id="resume-content">${skeleton(6)}</div>
        `, 'resumes');
        Layout.bindEvents();
    ...
    render(data) {
        ...
        const summary = r.summary?.content || '';

        container.innerHTML = `
        <div class="resume-builder">
            <section class="card">
                <header class="card__header"><h2 class="card__title">Personal Information</h2></header>
                <div class="card__body">
                    <form id="form-personal" class="settings-form" novalidate>
                        <div class="form-row form-row--2col">
                            <div class="form-group">
                                <label for="rb-first">First Name</label>
                                <input type="text" id="rb-first" value="${escapeHtml(personal.first_name || '')}" />
                            </div>
                            <div class="form-group">
                                <label for="rb-last">Last Name</label>
                                <input type="text" id="rb-last" value="${escapeHtml(personal.last_name || '')}" />
                            </div>
                        </div>
                        <div class="form-row form-row--2col">
                            <div class="form-group">
                                <label for="rb-email">Email</label>
                                <input type="email" id="rb-email" value="${escapeHtml(personal.email || '')}" />
                            </div>
                            <div class="form-group">
                                <label for="rb-phone">Phone</label>
                                <input type="tel" id="rb-phone" value="${escapeHtml(personal.phone || '')}" />
                            </div>
                        </div>
                        <div class="form-row form-row--2col">
                            <div class="form-group">
                                <label for="rb-location">Location</label>
                                <input type="text" id="rb-location" value="${escapeHtml(personal.location || '')}" />
                            </div>
                            <div class="form-group">
                                <label for="rb-jobtitle">Job Title</label>
                                <input type="text" id="rb-jobtitle" value="${escapeHtml(personal.job_title || '')}" />
                            </div>
                        </div>
                        <div class="form-row form-row--2col">
                            <div class="form-group">
                                <label for="rb-website">Website</label>
                                <input type="url" id="rb-website" value="${escapeHtml(personal.website || '')}" />
                            </div>
                            <div class="form-group">
                                <label for="rb-linkedin">LinkedIn</label>
                                <input type="url" id="rb-linkedin" value="${escapeHtml(personal.linkedin || '')}" />
                            </div>
                        </div>
            ...
            <section class="card">
                <header class="card__header"><h2 class="card__title">Professional Summary</h2></header>
                <div class="card__body">
                    <form id="form-summary" class="settings-form" novalidate>
                        <div class="form-group">
                            <textarea id="rb-summary" rows="5" placeholder="Write a brief professional summary...">${escapeHtml(summary)}</textarea>
                        </div>
            ...
            ${this._renderSectionList('Experience', r.experience || [], 'experience')}
            ${this._renderSectionList('Education', r.education || [], 'education')}
            ${this._renderSectionList('Skills', r.skills || [], 'skills')}
            ${this._renderSectionList('Projects', r.projects || [], 'projects')}
            ${this._renderSectionList('Certifications', r.certifications || [], 'certifications')}
        </div>`;

        this._bindPersonalForm();
        this._bindSummaryForm();
        this._bindSectionActions();
    },

    _renderSectionList(title, items, key) {
        const isEmpty = !items.length;
        const itemsHtml = items.map(item => {
            const label = item.company || item.institution || item.name || item.title || item.organization || 'Entry';
            const sub = item.position || item.degree || item.level || item.issuer || item.role || '';
            return `
            <div class="list-item" data-entry-id="${item.id}" data-section="${key}">
                <div class="list-item__info">
                    <span class="list-item__title">${escapeHtml(label)}</span>
                    ${sub ? `<span class="list-item__meta">${escapeHtml(sub)}</span>` : ''}
                </div>
                <div class="list-item__actions">
                    <button class="btn btn--sm btn--ghost btn--danger-text" data-delete-entry="${item.id}" data-section="${key}" aria-label="Delete entry">Delete</button>
                </div>
            </div>`;
        }).join('');
        ...
        <section class="card">
            <header class="card__header">
                ...
                <button class="btn btn--sm btn--secondary" data-add-section="${key}">+ Add</button>
            </header>
            <div class="card__body">
                ${isEmpty ? `<p class="text-muted">No ${title.toLowerCase()} added yet.</p>` : itemsHtml}
            </div>
        </section>`;
    },
    ...
    _bindPersonalForm() {
        ...
        form.addEventListener('submit', async e => {
            ...
            btn.textContent = 'Saving...';
            try {
                await API.put(`/api/resumes/${this._id}/personal`, {
                    first_name: $('#rb-first').value.trim(),
                    last_name: $('#rb-last').value.trim(),
                    email: $('#rb-email').value.trim(),
                    phone: $('#rb-phone').value.trim(),
                    location: $('#rb-location').value.trim(),
                    job_title: $('#rb-jobtitle').value.trim(),
                    website: $('#rb-website').value.trim(),
                    linkedin: $('#rb-linkedin').value.trim(),
                });
    ...
    _bindSummaryForm() {
        ...
        form.addEventListener('submit', async e => {
            ...
            btn.textContent = 'Saving...';
            try {
                await API.put(`/api/resumes/${this._id}/summary`, {
                    content: $('#rb-summary').value.trim(),
                });
    ...
    _bindSectionActions() {
        $$('[data-add-section]').forEach(btn => {
            btn.addEventListener('click', async () => {
                const section = btn.dataset.addSection;
                try {
                    await API.post(`/api/resumes/${this._id}/${section}`, {});
        ...
        $$('[data-delete-entry]').forEach(btn => {
            btn.addEventListener('click', () => {
                const entryId = btn.dataset.deleteEntry;
                const section = btn.dataset.section;
                Modal.confirm('Delete this entry?', async () => {
                    try {
                        await API.del(`/api/resumes/${this._id}/${section}/${entryId}`);
